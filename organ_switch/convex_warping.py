...
def swap_orgin(source_path, target_path, tag, source_is_read, source_landmarks):
    ...
    toc = time.time()
    # print("In convex-warping : Read landmarks took " + format(toc - tic) + " seconds.")

    M = affine_matrix(landmark1[align], landmark2[align])

    mask = get_organ_mask_mp(target, tag)
    warp_mask = warp_im(mask, M, source.shape)
    combined_mask = np.max([get_organ_mask_mp(source, tag), warp_mask],
                              axis=0)

    tic = time.time()
    # print("In convex-warping : Get mask took " + format(tic - toc) + " seconds.")
    ...
    toc = time.time()
    # print("In convex-warping : Color correction took " + format(toc - tic) + " seconds.")
...
if __name__ == '__main__':
    ...
    path2 = 'warping_sample/sample1/nose.png'
